                ...
                <x-ui.table.tr :key="$customer->id">
                    ...
                    <x-ui.table.td class="text-right pr-6">
                        <div class="flex items-center justify-end gap-2">
                            <x-ui.button size="sm" variant="ghost" icon="pencil-square" wire:click="$dispatch('editCustomer', { id: '{{ $customer->id }}' })" class="text-neutral-500 hover:text-primary-600" aria-label="Edit Pelanggan" />
                            <x-ui.button size="sm" variant="ghost" icon="trash" wire:click="deleteCustomer('{{ $customer->id }}')" wire:confirm="Yakin ingin menghapus pelanggan ini?" class="text-neutral-500 hover:text-danger-600 hover:bg-danger-50" aria-label="Hapus Pelanggan" />
                        </div>
                    </x-ui.table.td>
                </x-ui.table.tr>
            @empty
                ...
                <div class="p-4 flex flex-col gap-3 hover:bg-neutral-50 transition-colors">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <span class="text-body font-medium text-neutral-800 block">{{ $customer->name }}</span>
                                <span class="text-xs text-neutral-500">{{ $customer->npwp ? 'NPWP: '.$customer->npwp : 'Personal' }}</span>
                            </div>
                        </div>
                        <div>
                            @if($customer->active)
                    ...
                    <div class="grid grid-cols-2 gap-2 text-sm">
                        <div>
                            <span class="text-neutral-500 block text-xs">Kontak</span>
                            <span class="text-neutral-700 font-medium block">{{ $customer->phone ?? '-' }}</span>
                            <span class="text-neutral-500 text-xs">{{ $customer->email ?? '-' }}</span>
                        </div>
                        <div>
                            <span class="text-neutral-500 block text-xs">Loyalty Points</span>
                            <span class="text-primary-600 font-bold">{{ number_format($customer->loyalty_points) }} pts</span>
                        </div>
                    </div>
